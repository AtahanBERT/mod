...
exports.run = async(client, message, args, reply) => {
    ...
    if(!kullanici.kickable || message.member.roles.highest.position <= kullanici.roles.highest.position) return reply(`Bu kullanici sizle ayni/ust yetkidedir.`)
...
reply(`${kullanici} adli kullaniciyi sunucudan attim.`)
}
